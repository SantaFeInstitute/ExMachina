#JHM 17June2018 
#does data analysis/graphing on a directory of separate HawkDove runs
...
def plotIt(plt,data,title):
  ...
  plt.plot(data[:,0], data[:,5], linestyle = ':', label='<01 10> or\n<10 01>')
...
for dname in dfiles:
  ...
  for name in files: #for each file in the directory
    ...
    for line in fh: #for each line in the file
      ...
      x = line.find("5.000,")#get the (V-C)/2 parameter
      if (x != -1):
        comma = line[x+8:].find(",") #find the second comma
        c = float(line[8+comma+1:]) #grab the value after that comma
    fh.close()
    ...
    #print(name,tstates,rop,c,x00,x01,x10,x11,round(termcycletime/(counter-2),3),sep=", ")

    data.append([c,x00,x01,x10,x11,termcycletime/(counter-2)]) #add to the data array

  data = np.asarray(data) #convert to np arrays for processing and plotting
  data = data[data[:,0].argsort()] #sort by C value (in the 0th column)
  #print out the files
  print("\nConflict",str(tstates)+"s",str(rop)+"r")
  print(data)
  #plot out the individual files
  plotIt(plt,data,"CONFLICT "+str(tstates)+"s "+str(rop)+"r")
  finalizePlot(plt,"CONFLICT"+str(tstates)+"s"+str(rop)+"r")
...
for s in range(1,6):
  ...
  for r in range(0,3):
    ...
    plotIt(plt,data,"CONFLICT "+str(s)+"s "+str(round)+"r")
  finalizePlot(plt,"pCONFLICT"+str(s)+"s")

...
#this shoud be run in the Output directory
...
for dname in dfiles:
  ...
  for name in files: #for each file in the directory
    ...
    for line in fh: #for each line in the file
      ...
      x = line.find("0.000,") #get the B value, assumes C is 0.00
      if (x != -1):
        comma = line[x:].find(",")
        B = float(line[comma+x+1:]) #pull off the reward value
      x = line.find(",0.000 ") #get the D value, assumes C is 0.00
      if (x != -1):
        comma = line[x+1:].find(",")
        D = float(line[x+1+comma+1:x+1+comma+10]) #pull off D value
    fh.close()
    ...
    print(name,tstates,rop,B,D,x00,x01+x10,x11,sep=", ")

    data.append([B,D,x00,x01+x10,x11]) #add to the data array

  data = np.asarray(data) #convert to np arrays for processing and plotting
  data = data[np.lexsort((data[:,1],data[:,0]))] #lexiographcally sort by B, then D
  ...
  Bvalues = np.unique(data[:,0]) #find the unique values of B (will be ordered from low to high)
  Dvalues = np.unique(data[:,1]) #find the unique values of D (will be ordered from low to high)

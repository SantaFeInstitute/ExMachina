...
for dname in dfiles:
  ...
  for name in files: #for each file in the directory
    ...
    h = np.zeros((2*endowment+1,endowment+1)) #h[good 0 0..20, good 1 0..10]
    sum = 0
...
#    np.histogram2d(adata,bdata,bins=21,normed=True)
    ...
    """
    plt.hist2d(adata,bdata,range=np.array([(0, 2*endowment+1),(0, 2*endowment+1)]), normed=True, cmap=plt.cm.Reds)
    #bins = np.arange(endowment+1)
    #plt.hist2d(adata,bdata,range=np.array([(bins),(bins)]), cmap=plt.cm.Reds)
    plt.xlim([0,2*endowment+1])
    plt.ylim([0,2*endowment+1])
    """

    plt.colorbar()
    plt.title("CCUE "+str(tstates)+"s "+str(rop)+"r "+str(endowment)+"e Gens"+str(gstart)+"--"+str(gend))
    plt.show() #doesn't show if PS is invoked
    plt.savefig("CCdiff"+str(tstates)+"s"+str(rop)+"r"+str(endowment)+"e.jpg",format="jpg") #create a PS file of the fig
